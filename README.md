...
Note about ROMs: You can use either 28c128 EEPROM or 27c512 EPROM, appropriate .bin files are in the MetaphoricV2/ROMS subdirectory. 27c256 EPROM would be enough to store two 16K banks with Oric-1 and Atmos ROMs, but it has different pinout - it would still work, but it wouldn't be possible to select ROM banks and only the high bank would be available. However, if you happen to have only 27c256 EPROMs, you may cut the trace between the ROM pins 27 and 28 (on the back of the PCB) and run a bodge wire from ROM pin 1 to ROM pin 27; this should make it possible to use a 27c256 EPROM.
